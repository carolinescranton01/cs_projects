...
### NOTE: Code is written in blue. Lines of code which are NOT loops (ie commands like cd, ls, rm, cp, and more) are written with a bracket > before the code. Do not include the bracket > when typing the command into terminal
If the tutorial says:
>mkdir sequences
You will type:
mkdir sequences
