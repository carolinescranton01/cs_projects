...
df = pd.read_csv('16S_Illumina_taxonomy_IgA.csv')
...
if __name__ == "__main__":
    ...
    result_all.to_csv('out_all_IgA.csv', index=False)
    print("Wrote out_all.csv")

    # family-only
    result_family = calculate_log_ratio_family(df)
    result_family.to_csv('out_family_IgA.csv', index=False)
    print("Wrote out_family.csv")

    # genus-only
    result_genus = calculate_log_ratio_genus(df)
    result_genus.to_csv('out_genus_IgA.csv', index=False)
    print("Wrote out_genus_IgA.csv")
